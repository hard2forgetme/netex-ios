...
APP_BUNDLE_ID="${NETEX_BUNDLE_IDENTIFIER:-io.github.hard2forgetme.netexios}"
...
LAUNCH_OUTPUT="$(xcrun simctl launch --terminate-running-process "$SIM_ID" "$APP_BUNDLE_ID" --netex-reset --netex-url "$PROFILE_URL")"
sleep 2
xcrun simctl io "$SIM_ID" screenshot "$SCREENSHOT" >/dev/null
xcrun simctl spawn "$SIM_ID" log show --last 3m --style compact --predicate 'subsystem == "io.github.hard2forgetme.netexios"' >"$DEVICE_LOG" || true
